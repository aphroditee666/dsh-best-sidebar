window.__ModuleLoader__.load({
	id: "dsh-external/@aphroditee666/dsh-best-sidebar",
	factory: (require) => {
		var module = { exports: {} };
		...
		*/
		const SIDEBAR_SERVICE_VERSION = "0.15.0";
		/**
		...
		*/
		function createBetterSidebarService(store) {
			...
			const registerTab = (descriptor) => {
				if (tabs.has(descriptor.id)) throw new Error(`[dsh-best-sidebar] tab type "${descriptor.id}" already registered`);
			...
			const registerFileViewer = (descriptor) => {
				if (viewers.has(descriptor.id)) throw new Error(`[dsh-best-sidebar] file viewer "${descriptor.id}" already registered`);
		...
		*  global shared with chunk-bundle copies, then the rc.7 page global). */
		...
		const defaultScriptLoader = (src) => new Promise((resolve, reject) => {
			...
			el.addEventListener("error", () => {
				el.remove();
				reject(/* @__PURE__ */ new Error(`[dsh-best-sidebar] chunk script ${src} failed to load`));
			}, { once: true });
		...
		async function buildExternalsRequire(modules) {
			...
			externalsRequire = (spec) => {
				if (!table.has(spec)) throw new Error(`[dsh-best-sidebar] chunk require('${spec}') missed the module table`);
		...
		*/
		async function loadChunk(name) {
			...
			task = (async () => {
				...
				if (modules === void 0) throw new Error(`[dsh-best-sidebar] chunk "${name}": client module system unavailable`);
				await scriptLoader(CHUNK_URL(name));
				const factory = chunkRegistry()[name];
				if (typeof factory !== "function") throw new Error(`[dsh-best-sidebar] chunk "${name}" script did not register its factory`);
		...
		*/
		...
		const zh = {
			...
			addPluginsBrowseMore: "在 GitHub 上浏览更多插件（topic: dsh-best-sidebar）",
		...
		const en = {
			...
			addPluginsBrowseMore: "Browse more plugins on GitHub (topic: dsh-best-sidebar)",
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:D:\BaiduSyncdisk\obsidian\Github\clyde_dsh\dsh_projects\plugins\dsh-best-sidebar\src\client\sidebar.module.css.mjs
		const css$4 = "[data-dsh-panel-host]{z-index:40;pointer-events:none;position:fixed;inset:0}[data-dsh-panel-host][data-dsh-panel-host-degraded]{position:absolute;top:0;left:0}._8JD7uW_toggleCluster{top:calc(3px + env(safe-area-inset-top));z-index:45;pointer-events:auto;flex-direction:row;gap:4px;display:flex;position:absolute;right:10px}._8JD7uW_panel:not(._8JD7uW_panelHidden) ._8JD7uW_tabBar{padding-right:72px}._8JD7uW_toggleButton{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out), color var(--ds-transition-duration-slow) var(--ds-ease-in-out);background:0 0;border:none;border-radius:50%;justify-content:center;align-items:center;display:flex}._8JD7uW_toggleButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._8JD7uW_toggleButton:disabled{opacity:.4;cursor:default}._8JD7uW_panel{z-index:40;pointer-events:auto;background:var(--dsw-alias-bg-layer-1);border-left:1px solid var(--dsw-alias-border-l2);padding-bottom:env(safe-area-inset-bottom);transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), width var(--ds-transition-duration-slow) var(--ds-ease-in-out);flex-direction:column;display:flex;position:absolute;top:0;bottom:0;right:0}._8JD7uW_panelHidden{pointer-events:none;visibility:hidden;transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), width var(--ds-transition-duration-slow) var(--ds-ease-in-out), visibility 0s linear var(--ds-transition-duration-slow);transform:translate(102%)}._8JD7uW_panel[data-dragging]{transition:none}._8JD7uW_panelResize{cursor:col-resize;z-index:2;touch-action:none;width:8px;position:absolute;top:0;bottom:0;left:-4px}._8JD7uW_panelResizeActive{background:var(--dsw-alias-interactive-bg-hover-accent)}._8JD7uW_panelBody{flex:1;min-width:0;min-height:0;display:flex}._8JD7uW_bottomPanel{z-index:40;background:var(--dsw-alias-bg-layer-1);border-top:1px solid var(--dsw-alias-border-l2);pointer-events:auto;padding-bottom:env(safe-area-inset-bottom);transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), height var(--ds-transition-duration-slow) var(--ds-ease-in-out);flex-direction:column;display:flex;position:absolute;bottom:0}._8JD7uW_bottomPanelHidden{pointer-events:none;visibility:hidden;transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), height var(--ds-transition-duration-slow) var(--ds-ease-in-out), visibility 0s linear var(--ds-transition-duration-slow);transform:translateY(102%)}._8JD7uW_bottomPanel[data-dragging]{transition:none}._8JD7uW_panel,._8JD7uW_bottomPanel{contain:layout style}body[data-dsh-sidebar-dragging] ._8JD7uW_panel,body[data-dsh-sidebar-dragging] ._8JD7uW_bottomPanel{will-change:transform}._8JD7uW_bottomResize{cursor:row-resize;z-index:2;touch-action:none;height:8px;position:absolute;top:-4px;left:0;right:0}._8JD7uW_bottomResizeActive{background:var(--dsw-alias-interactive-bg-hover-accent)}._8JD7uW_bottomClose{z-index:4;width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex;position:absolute;top:3px;right:6px}._8JD7uW_bottomClose:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._8JD7uW_bottomPanel ._8JD7uW_tabBar{padding-right:40px}._8JD7uW_toggleCluster,._8JD7uW_toggleButton,._8JD7uW_tabBar{-webkit-app-region:no-drag}body[data-dsh-title-bar-compat] ._8JD7uW_toggleCluster{top:calc(var(--dsh-title-bar-strip,40px) + 3px)}body[data-dsh-title-bar-compat] ._8JD7uW_panel{padding-top:var(--dsh-title-bar-strip,40px)}._8JD7uW_cornerHandle{left:-6px;bottom:calc(var(--dsh-sidebar-height,0px) + 6px);z-index:2;cursor:nwse-resize;touch-action:none;width:12px;height:12px;position:absolute}._8JD7uW_cornerHandle:hover,._8JD7uW_cornerHandle[data-dragging]{background:var(--dsw-alias-interactive-bg-hover-accent)}._8JD7uW_iconButton{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}._8JD7uW_iconButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._8JD7uW_iconButton:disabled{opacity:.4;cursor:default}._8JD7uW_workbench,._8JD7uW_split{flex:1;min-width:0;min-height:0;display:flex}._8JD7uW_splitRow{flex-direction:row}._8JD7uW_splitCol{flex-direction:column}._8JD7uW_splitChild{display:flex;position:relative;overflow:hidden}._8JD7uW_divider{z-index:3;touch-action:none;flex:none;position:relative}._8JD7uW_dividerRow:after,._8JD7uW_dividerCol:after{content:\"\";background:var(--dsw-alias-border-l2);transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out);position:absolute}._8JD7uW_dividerRow{cursor:col-resize;width:7px;margin:0 -2px}._8JD7uW_dividerRow:after{width:1px;top:0;bottom:0;left:50%;transform:translate(-50%)}._8JD7uW_dividerCol{cursor:row-resize;height:7px;margin:-2px 0}._8JD7uW_dividerCol:after{height:1px;top:50%;left:0;right:0;transform:translateY(-50%)}._8JD7uW_divider:hover:after,._8JD7uW_dividerActive:after{background:var(--dsw-alias-interactive-bg-hover-accent)}._8JD7uW_pane{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;position:relative}._8JD7uW_paneDrop{outline:1px solid var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}._8JD7uW_dropOverlay{z-index:6;pointer-events:none;background:var(--dsw-alias-interactive-bg-hover-accent);opacity:.5;position:absolute}._8JD7uW_dropLeft{width:25%;top:0;bottom:0;left:0}._8JD7uW_dropRight{width:25%;top:0;bottom:0;right:0}._8JD7uW_dropUp{height:25%;top:0;left:0;right:0}._8JD7uW_dropDown{height:25%;bottom:0;left:0;right:0}._8JD7uW_dropCenter{outline:2px dashed var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-2px;background:0 0;inset:25%}._8JD7uW_paneContent{flex-direction:column;flex:1;min-height:0;display:flex;overflow:hidden}._8JD7uW_paneTab{flex-direction:column;flex:1;min-height:0;display:flex}._8JD7uW_paneTabHidden{display:none}._8JD7uW_paneEmptyCards{flex:1;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));align-content:start;gap:8px;min-height:0;padding:12px;display:grid;overflow:hidden}._8JD7uW_paneCard{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxs-strong-12);cursor:pointer;text-align:center;border-radius:8px;flex-direction:column;justify-content:center;align-items:center;gap:6px;padding:12px 8px;display:flex}._8JD7uW_paneCard:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-border-l2)}._8JD7uW_paneCard:disabled{opacity:.45;cursor:default}._8JD7uW_tabBar{border-bottom:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);flex:none;align-items:stretch;height:34px;display:flex}._8JD7uW_tabBarDrop{outline:1px dashed var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}._8JD7uW_tabList{scrollbar-width:none;flex:1;min-width:0;display:flex;overflow-x:auto}._8JD7uW_tabList::-webkit-scrollbar{display:none}._8JD7uW_tab{min-width:64px;max-width:160px;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-secondary);border-right:1px solid var(--dsw-alias-border-l1);cursor:pointer;user-select:none;background:0 0;flex:none;align-items:center;gap:4px;padding:0 4px 0 10px;display:flex}._8JD7uW_tab:hover{background:var(--dsw-alias-interactive-bg-hover)}._8JD7uW_tabActive{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-active)}._8JD7uW_tabTitle{text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0;overflow:hidden}._8JD7uW_tabBadge{min-width:16px;height:15px;font:var(--dsw-font-xxxs-strong-11);background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-brand-primary);border-radius:8px;flex:none;justify-content:center;align-items:center;padding:0 4px;display:inline-flex}._8JD7uW_tabClose{width:18px;height:18px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:none;border-radius:4px;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}._8JD7uW_tabClose:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._8JD7uW_tabBarPlus{background:var(--dsw-alias-bg-layer-1);width:22px;height:22px;color:var(--dsw-alias-label-tertiary);cursor:pointer;border:none;border-radius:5px;flex:none;justify-content:center;align-self:center;align-items:center;margin:0 6px;padding:0;display:inline-flex;position:sticky;right:0}._8JD7uW_tabBarPlus:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._8JD7uW_explorer{flex-direction:column;flex:1;min-height:0;display:flex}._8JD7uW_explorerHeader{flex:none;justify-content:space-between;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}._8JD7uW_explorerRoot{font:var(--dsw-font-s-14);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._8JD7uW_explorerBody{flex:1;min-height:0;padding:2px 6px 8px;overflow:hidden auto}._8JD7uW_explorerRow{box-sizing:border-box;width:100%;max-width:100%;height:34px;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;white-space:nowrap;animation:_8JD7uW_dsh-row-in .15s var(--ds-ease-in-out);background:0 0;border:none;border-radius:8px;align-items:center;gap:6px;padding:0 8px;display:flex}._8JD7uW_explorerRow:hover{background:var(--dsw-alias-interactive-bg-hover)}._8JD7uW_explorerDir{font:var(--dsw-font-s-strong-14)}._8JD7uW_explorerHidden{opacity:.45}._8JD7uW_explorerSymlink{color:var(--dsw-alias-label-tertiary);flex:none}._8JD7uW_explorerBroken ._8JD7uW_explorerName{color:var(--dsw-alias-state-error-primary)}._8JD7uW_explorerName{text-overflow:ellipsis;white-space:nowrap;min-width:0;overflow:hidden}._8JD7uW_explorerRef{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);height:20px;color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;border-radius:999px;flex:none;align-items:center;padding:0 8px;display:none}._8JD7uW_explorerRef:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._8JD7uW_explorerRow:hover ._8JD7uW_explorerRef,._8JD7uW_explorerRow:focus-within ._8JD7uW_explorerRef{display:inline-flex}._8JD7uW_explorerCopied{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}._8JD7uW_explorerError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);cursor:default}@keyframes _8JD7uW_dsh-row-in{0%{opacity:0}}._8JD7uW_explorerEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;padding:16px}._8JD7uW_explorerRowDropTarget{background:var(--dsw-alias-interactive-bg-hover);outline:1px dashed var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}._8JD7uW_uploadDropZone{z-index:1001;pointer-events:none;border:2px dashed var(--dsw-alias-interactive-bg-hover-accent);box-shadow:0 0 0 200vmax var(--dsw-alias-bg-mask-drop);animation:_8JD7uW_dsh-row-in .15s var(--ds-ease-in-out);border-radius:10px;justify-content:center;align-items:flex-start;padding:12px;display:flex;position:fixed}._8JD7uW_uploadDropHero{flex-direction:column;align-items:center;gap:10px;max-width:100%;padding-top:8px;display:flex}._8JD7uW_uploadDropZonePill{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);max-width:100%;box-shadow:var(--dsw-shadow-lv2);color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-strong-12);border-radius:999px;align-items:center;gap:6px;padding:6px 12px;display:flex}._8JD7uW_uploadDropZoneText{white-space:nowrap;text-overflow:ellipsis;overflow:hidden}._8JD7uW_uploadDropChatHint{z-index:1002;pointer-events:none;animation:_8JD7uW_dsh-row-in .15s var(--ds-ease-in-out);justify-content:center;align-items:center;padding:24px;display:flex;position:fixed;top:0;bottom:0;left:0}._8JD7uW_uploadDropChatCard{text-align:center;max-width:100%;color:var(--dsw-alias-label-primary);font:var(--dsw-font-s-strong-14);flex-direction:column;align-items:center;gap:12px;display:flex}._8JD7uW_uploadOverlay{z-index:30;background:var(--dsw-alias-bg-mask-1);backdrop-filter:var(--dsw-mask-blur);animation:_8JD7uW_dsh-row-in .15s var(--ds-ease-in-out);justify-content:center;align-items:center;display:flex;position:absolute;inset:0}._8JD7uW_uploadOverlayCard{border:1px solid var(--dsw-alias-border-inverted);background:var(--dsw-alias-bg-layer-2);min-width:280px;max-width:min(420px,100% - 48px);box-shadow:var(--dsw-shadow-lv3);border-radius:24px;flex-direction:column;gap:12px;padding:20px 24px;display:flex}._8JD7uW_uploadOverlayTitle{font:var(--dsw-font-s-strong-14);color:var(--dsw-alias-label-primary);align-items:center;gap:8px;display:flex}._8JD7uW_uploadOverlayTitle>svg{flex:none}._8JD7uW_uploadOverlayTitle>span{white-space:nowrap;text-overflow:ellipsis;min-width:0;overflow:hidden}._8JD7uW_uploadOverlayProgress{background:var(--dsw-alias-border-l2);border-radius:3px;height:6px;overflow:hidden}._8JD7uW_uploadOverlayProgressFill{background:var(--dsw-alias-interactive-bg-hover-accent);height:100%;transition:width .15s var(--ds-ease-in-out);border-radius:3px}._8JD7uW_uploadOverlayStatus{min-height:1em;font:var(--dsw-font-xxs-12);font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-tertiary);white-space:nowrap;text-overflow:ellipsis;overflow:hidden}._8JD7uW_uploadOverlayCancel{border:1px solid var(--dsw-alias-border-l2);height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-strong-12);cursor:pointer;background:0 0;border-radius:8px;align-self:flex-end;padding:0 14px}._8JD7uW_uploadOverlayCancel:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-border-l2)}._8JD7uW_uploadOverlayCancel:disabled{opacity:.4;cursor:default}._8JD7uW_editor{flex-direction:column;flex:1;min-height:0;display:flex}._8JD7uW_editorHeader{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:6px;padding:4px 8px;display:flex}._8JD7uW_editorTitle{min-width:0;font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}._8JD7uW_editorPathInput{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 10px}._8JD7uW_editorPathInput:focus{border-color:var(--dsw-alias-border-l2);outline:none}._8JD7uW_editorTreeToggleActive{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-active)}._8JD7uW_editorBody{flex:1;min-height:0;display:flex}._8JD7uW_editorMain{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex}._8JD7uW_editorTreeDock{border-left:1px solid var(--dsw-alias-border-l1);flex:none;min-height:0;display:flex;position:relative}._8JD7uW_editorTreeResize{cursor:col-resize;touch-action:none;z-index:3;width:6px;position:absolute;top:0;bottom:0;left:0}._8JD7uW_editorTreeResize:hover{background:var(--dsw-alias-border-l2)}._8JD7uW_editorTreePanel{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;position:relative}._8JD7uW_editorTreePanelFull{flex:1}._8JD7uW_editorTreeSearch{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:4px;padding:6px 8px;display:flex}._8JD7uW_editorSearchInput{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;height:26px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 10px}._8JD7uW_editorSearchInput:focus{border-color:var(--dsw-alias-border-l2);outline:none}._8JD7uW_editorSearchHint{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);padding:8px 12px}._8JD7uW_editorSearchResult{width:100%;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);text-align:left;cursor:pointer;text-overflow:ellipsis;white-space:nowrap;background:0 0;border:none;border-radius:6px;padding:4px 8px;display:block;overflow:hidden}._8JD7uW_editorSearchResult:hover{background:var(--dsw-alias-interactive-bg-hover)}._8JD7uW_editorStatus{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}._8JD7uW_editorStatusError{color:var(--dsw-alias-state-error-primary)}._8JD7uW_dirtyDot{background:var(--dsw-alias-state-warn-primary);border-radius:50%;flex:none;width:7px;height:7px}._8JD7uW_editorPlaceholder{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;flex:1;justify-content:center;align-items:center;padding:16px;display:flex}._8JD7uW_orphanedType{opacity:.7;overflow-wrap:anywhere;margin-top:8px;font-size:12px;display:block}._8JD7uW_editorBinary{text-align:center;flex-direction:column;flex:1;justify-content:center;align-items:center;gap:12px;padding:24px 16px;display:flex}._8JD7uW_editorBinaryNotice{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}._8JD7uW_editorDownloadLink{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-strong-12);cursor:pointer;transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out), border-color var(--ds-transition-duration-slow) var(--ds-ease-in-out);border-radius:6px;align-items:center;gap:6px;padding:6px 14px;text-decoration:none;display:inline-flex}._8JD7uW_editorDownloadLink:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-border-l2)}._8JD7uW_editorError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);padding:12px 16px}._8JD7uW_editorBanner{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex:none;padding:4px 12px}._8JD7uW_sandboxStatus{font:var(--dsw-font-xxxs-11);flex:none;align-items:center;gap:8px;padding:4px 10px;display:flex}._8JD7uW_sandboxStatusOn{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);border-bottom:1px solid var(--dsw-alias-border-l1)}._8JD7uW_sandboxStatusOff{color:var(--dsw-alias-state-error-primary);background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 10%, transparent);border-bottom:1px solid color-mix(in srgb, var(--dsw-alias-state-error-primary) 45%, transparent)}._8JD7uW_sandboxDot{background:var(--dsw-alias-state-success-primary);border-radius:50%;flex:none;width:6px;height:6px}._8JD7uW_sandboxStatusOff ._8JD7uW_sandboxDot{background:var(--dsw-alias-state-error-primary)}._8JD7uW_sandboxStatusText{text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0;overflow:hidden}._8JD7uW_sandboxAction{border:1px solid var(--dsw-alias-border-l2);font:inherit;color:inherit;cursor:pointer;background:0 0;border-radius:6px;flex:none;padding:2px 8px}._8JD7uW_sandboxAction:hover{background:var(--dsw-alias-interactive-bg-hover)}._8JD7uW_editorHtml{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}._8JD7uW_browser{flex-direction:column;flex:1;min-height:0;display:flex}._8JD7uW_browserBar{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:4px;padding:6px 8px;display:flex}._8JD7uW_browserInput{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 10px}._8JD7uW_browserInput:focus{border-color:var(--dsw-alias-border-l2);outline:none}._8JD7uW_browserMessage{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex:none;padding:4px 12px}._8JD7uW_browserFrame{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}._8JD7uW_browserStart{text-align:center;min-height:0;font:var(--dsw-font-xs-13);color:var(--dsw-alias-label-tertiary);flex:1;justify-content:center;align-items:center;padding:20px;display:flex}._8JD7uW_browserBlocked{text-align:center;min-height:0;color:var(--dsw-alias-state-warn-primary);flex-direction:column;flex:1;justify-content:center;align-items:center;gap:6px;padding:24px;display:flex}._8JD7uW_browserBlockedTitle{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary)}._8JD7uW_browserBlockedDesc{max-width:280px;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-secondary)}._8JD7uW_browserBlockedActions{gap:8px;margin-top:6px;display:flex}._8JD7uW_browserBlockedButton{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxxs-11);cursor:pointer;border-radius:6px;padding:4px 12px}._8JD7uW_browserBlockedButton:hover{background:var(--dsw-alias-interactive-bg-hover)}._8JD7uW_editorCm{background:0 0;flex:1;min-height:0;overflow:hidden}._8JD7uW_editorCmHidden{display:none}._8JD7uW_editorCm .cm-editor{height:100%}._8JD7uW_editorCm .cm-editor.cm-focused{outline:none}._8JD7uW_editorModeToggle{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);border-radius:6px;flex:none;align-items:center;gap:2px;padding:2px;display:inline-flex}._8JD7uW_editorModeButton{color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-11);cursor:pointer;background:0 0;border:none;border-radius:4px;padding:2px 8px}._8JD7uW_editorModeButton:hover{color:var(--dsw-alias-label-primary)}._8JD7uW_editorModeActive{background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary)}._8JD7uW_editorImageWrap{flex:1;justify-content:center;align-items:center;min-height:0;padding:12px;display:flex;overflow:auto}._8JD7uW_editorImage{object-fit:contain;max-width:100%;max-height:100%}._8JD7uW_editorMd{min-height:0;font:var(--dsw-font-xs-13);flex:1;padding:10px 14px;overflow-y:auto}._8JD7uW_mermaidWrap{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);border-radius:6px;margin:6px 0;overflow:hidden}._8JD7uW_mermaidHeader{border-bottom:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);justify-content:space-between;align-items:center;gap:6px;padding:4px 8px;display:flex}._8JD7uW_mermaidInfo{font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-tertiary)}._8JD7uW_mermaidCopy{height:20px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxxs-11);cursor:pointer;background:0 0;border:none;border-radius:4px;align-items:center;gap:4px;padding:0 6px;display:inline-flex}._8JD7uW_mermaidCopy:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._8JD7uW_mermaidBody{cursor:zoom-in;justify-content:center;padding:10px;display:flex;overflow:auto}._8JD7uW_mermaidBody svg{max-width:100%;height:auto}._8JD7uW_mermaidError{border-bottom:1px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-state-error-primary);font:var(--dsw-font-xxxs-11);padding:6px 10px}._8JD7uW_mermaidCode{font:var(--dsw-font-xxxs-11);margin:0;padding:8px 10px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;overflow:auto}._8JD7uW_mermaidMarkdown .md-code-block[data-mermaid-processed]{display:contents}._8JD7uW_mermaidModal{z-index:1000;background:var(--dsw-alias-bg-mask-1);backdrop-filter:blur(2px);flex-direction:column;justify-content:center;align-items:center;display:flex;position:fixed;inset:0}._8JD7uW_mermaidModalToolbar{z-index:10;gap:8px;display:flex;position:absolute;top:16px;right:16px}._8JD7uW_mermaidModalButton{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);width:36px;height:36px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xs-strong-13);cursor:pointer;border-radius:8px;justify-content:center;align-items:center;display:inline-flex}._8JD7uW_mermaidModalButton:hover{background:var(--dsw-alias-interactive-bg-hover)}._8JD7uW_mermaidModalStage{justify-content:center;align-items:center;width:90vw;height:80vh;display:flex;position:relative;overflow:hidden}._8JD7uW_mermaidModalStage svg{cursor:grab;transform-origin:50%;user-select:none;-webkit-user-drag:none;background:var(--dsw-alias-bg-layer-1);border-radius:12px;max-width:none;max-height:none;padding:16px}._8JD7uW_mermaidModalStage svg:active{cursor:grabbing}._8JD7uW_mermaidModalHint{color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-11);pointer-events:none;position:absolute;bottom:16px;left:50%;transform:translate(-50%)}._8JD7uW_selectionPopup{z-index:60;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxxs-strong-11);white-space:nowrap;cursor:pointer;border-radius:6px;align-items:center;padding:0 10px;display:inline-flex;position:fixed;transform:translate(-50%,calc(-100% - 8px))}._8JD7uW_selectionPopup:hover{background:var(--dsw-alias-interactive-bg-hover)}._8JD7uW_editorPdf{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-height:0;display:flex}._8JD7uW_editorPdfToolbar{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;justify-content:flex-end;padding:6px 8px;display:flex}._8JD7uW_editorPdfStage{flex:1;min-height:0;display:flex;position:relative}._8JD7uW_editorPdfFrame{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}._8JD7uW_editorPdfFrameBlocked{pointer-events:none}._8JD7uW_editorPdfDragShield{z-index:4;pointer-events:none;background:0 0;position:absolute;inset:0}._8JD7uW_editorPdfDragShieldActive{pointer-events:auto}body[data-dsh-tab-dragging] ._8JD7uW_editorPdfFrame{pointer-events:none!important}body[data-dsh-tab-dragging] ._8JD7uW_editorPdfDragShield{pointer-events:auto!important}._8JD7uW_terminalWrap{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-height:0;display:flex;position:relative}._8JD7uW_terminal{flex:1;min-height:0;padding:6px 4px 6px 8px}._8JD7uW_terminal .xterm{height:100%}._8JD7uW_terminalBanner{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex-wrap:wrap;flex:none;align-items:center;gap:8px;padding:3px 10px;display:flex}._8JD7uW_terminalBannerUrl{word-break:break-all;opacity:.85;flex-basis:100%;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}._8JD7uW_boundaryError{z-index:50;background:var(--dsw-alias-bg-layer-1);border-left:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);flex-direction:column;align-items:flex-start;gap:8px;padding:16px;display:flex;position:fixed;top:0;bottom:0;right:0;overflow:auto}._8JD7uW_terminalRetry{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;border-radius:999px;flex:none;padding:1px 8px}._8JD7uW_terminalRetry:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._8JD7uW_terminalDepsBanner{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex-direction:column;flex:none;gap:6px;padding:10px;display:flex}._8JD7uW_terminalDepsTitle{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-state-warn-primary)}._8JD7uW_terminalDepsHint{opacity:.9}._8JD7uW_terminalDepsCommandRow{align-items:flex-start;gap:8px;display:flex}._8JD7uW_terminalRepairCommand{white-space:pre-wrap;word-break:break-all;user-select:text;min-width:0;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:4px;flex:1;max-height:160px;margin:0;padding:6px 8px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;line-height:1.5;overflow:auto}._8JD7uW_terminalDepsNote{opacity:.85}._8JD7uW_terminalDepsActions{align-items:center;gap:8px;display:flex}._8JD7uW_tabBoundaryError{min-height:0;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);flex-direction:column;flex:1;align-items:flex-start;gap:8px;padding:12px 16px;display:flex;overflow:auto}._8JD7uW_git{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;overflow:hidden auto}._8JD7uW_gitHeader{flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}._8JD7uW_gitBranchSelect{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);min-width:0;height:26px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 6px}._8JD7uW_gitSection{border-top:1px solid var(--dsw-alias-border-l1)}._8JD7uW_gitSectionHeader{font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-tertiary);text-transform:uppercase;justify-content:space-between;align-items:center;padding:6px 12px 4px;display:flex}._8JD7uW_gitLink{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-brand-primary);cursor:pointer;background:0 0;border:none;padding:0}._8JD7uW_gitLink:hover:not(:disabled){text-decoration:underline}._8JD7uW_gitLink:disabled{opacity:.4;cursor:default}._8JD7uW_gitRow{min-height:34px;animation:_8JD7uW_dsh-row-in .15s var(--ds-ease-in-out);border-radius:8px;align-items:center;gap:6px;margin:0 6px;padding:0 8px;display:flex}._8JD7uW_gitRow:hover{background:var(--dsw-alias-interactive-bg-hover)}._8JD7uW_gitRowSelected{background:var(--dsw-alias-interactive-bg-active)}._8JD7uW_gitRowMain{cursor:pointer;text-align:left;background:0 0;border:none;flex:1;align-items:center;gap:8px;min-width:0;padding:3px 0;display:flex}._8JD7uW_gitBadge{width:20px;height:16px;font:var(--dsw-font-xxxs-strong-11);background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-secondary);border-radius:4px;flex:none;justify-content:center;align-items:center;display:inline-flex}._8JD7uW_gitName{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}._8JD7uW_gitEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);padding:4px 12px 8px}._8JD7uW_gitPlaceholder{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;padding:16px}._8JD7uW_gitError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);white-space:pre-wrap;padding:8px 12px}._8JD7uW_gitDiff{border-top:1px solid var(--dsw-alias-border-l1);padding:8px}._8JD7uW_gitDiffTab{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;overflow:hidden auto}._8JD7uW_gitDiffTabHeader{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}._8JD7uW_gitDiffTabTitle{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}._8JD7uW_gitDiffFile{align-items:baseline;gap:6px;padding:8px 2px 2px;display:flex}._8JD7uW_gitDiffFilePath{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._8JD7uW_gitDiffFileOld{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;flex:none;max-width:40%;overflow:hidden}._8JD7uW_gitDiffFileTag{border:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);border-radius:999px;flex:none;padding:0 6px}._8JD7uW_gitDiffHunk{font:var(--dsw-font-markdown-code-block-small);color:var(--dsw-alias-label-tertiary);gap:8px;padding:3px 2px;display:flex}._8JD7uW_gitDiffHunkHeader{color:var(--dsw-alias-label-secondary);flex:none}._8JD7uW_gitDiffHunkSection{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._8JD7uW_gitDiffLine{font:var(--dsw-font-markdown-code-block-small);white-space:pre-wrap;overflow-wrap:anywhere;align-items:stretch;min-width:0;line-height:20px;display:flex}._8JD7uW_gitDiffNum{text-align:right;width:36px;color:var(--dsw-alias-label-tertiary);user-select:none;flex:none;padding-right:8px}._8JD7uW_gitDiffCode{flex:1;min-width:0;overflow:visible}._8JD7uW_gitDiffCtx{color:var(--dsw-alias-label-primary)}._8JD7uW_gitDiffDel{color:var(--dsw-alias-state-error-primary);background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent)}._8JD7uW_gitDiffAdd{color:var(--dsw-alias-state-success-primary);background:color-mix(in srgb, var(--dsw-alias-state-success-primary) 12%, transparent)}._8JD7uW_gitDiffMeta{padding-left:2px}._8JD7uW_gitDiffMetaText{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);font-style:italic}._8JD7uW_gitDiffExpand{width:100%;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-brand-primary);cursor:pointer;text-align:center;background:0 0;border:none;margin:4px 0;display:block}._8JD7uW_gitDiffExpand:hover{background:var(--dsw-alias-interactive-bg-hover)}._8JD7uW_gitConfirmDesc{font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);white-space:pre-wrap;margin:0}._8JD7uW_gitCommit{border-top:1px solid var(--dsw-alias-border-l1);align-items:center;gap:6px;padding:8px 12px;display:flex}._8JD7uW_gitCommitInput{flex:1;min-width:0}._8JD7uW_gitCommitButton{background:var(--dsw-alias-button-primary-fill);height:26px;color:var(--dsw-alias-label-primary-inverted);font:var(--dsw-font-xxs-strong-12);cursor:pointer;border:none;border-radius:6px;flex:none;padding:0 12px}._8JD7uW_gitCommitButton:hover:not(:disabled){background:var(--dsw-alias-button-primary-hover)}._8JD7uW_gitCommitButton:disabled{opacity:.45;cursor:default}._8JD7uW_gitLogRow{cursor:pointer;border-radius:8px;flex-direction:column;gap:2px;padding:5px 12px;display:flex}._8JD7uW_gitLogRow:hover{background:var(--dsw-alias-interactive-bg-hover)}._8JD7uW_gitLogLine1{align-items:baseline;gap:8px;min-width:0;display:flex}._8JD7uW_gitLogHash{font:var(--dsw-font-markdown-code-block-small);color:var(--dsw-alias-label-tertiary);flex:none}._8JD7uW_gitLogLine2{flex-wrap:wrap;align-items:center;gap:6px;min-width:0;display:flex}._8JD7uW_gitLogRef{border:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-brand-primary);white-space:nowrap;border-radius:999px;flex:none;padding:0 5px}._8JD7uW_gitLogSubject{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}._8JD7uW_gitLogMeta{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}._8JD7uW_gitLogMore{border:1px solid var(--dsw-alias-border-l2);width:calc(100% - 24px);font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border-radius:6px;margin:4px 12px 8px;padding:6px 0;display:block}._8JD7uW_gitLogMore:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._8JD7uW_gitLogMore:disabled{opacity:.5;cursor:default}._8JD7uW_producedRow{flex-wrap:wrap;align-items:center;gap:8px;padding:4px 0;display:flex}._8JD7uW_producedLabel{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}._8JD7uW_producedChip{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);max-width:200px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxs-12);cursor:pointer;border-radius:999px;align-items:center;gap:4px;padding:2px 8px;display:inline-flex;overflow:hidden}._8JD7uW_producedChip:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}._8JD7uW_producedChip span{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}._8JD7uW_producedMore{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}._8JD7uW_toggleButton:focus-visible,._8JD7uW_bottomClose:focus-visible,._8JD7uW_iconButton:focus-visible,._8JD7uW_tab:focus-visible,._8JD7uW_tabClose:focus-visible,._8JD7uW_tabBarPlus:focus-visible,._8JD7uW_paneCard:focus-visible,._8JD7uW_explorerRow:focus-visible,._8JD7uW_explorerRef:focus-visible,._8JD7uW_gitRowMain:focus-visible,._8JD7uW_gitLink:focus-visible,._8JD7uW_gitCommitButton:focus-visible,._8JD7uW_gitLogRow:focus-visible,._8JD7uW_gitLogMore:focus-visible,._8JD7uW_gitDiffExpand:focus-visible,._8JD7uW_terminalRetry:focus-visible,._8JD7uW_editorModeButton:focus-visible,._8JD7uW_editorDownloadLink:focus-visible,._8JD7uW_editorPptxButton:focus-visible,._8JD7uW_editorDocxZoomRange:focus-visible{outline:2px solid var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}@media (prefers-reduced-motion:reduce){._8JD7uW_panel,._8JD7uW_panelHidden,._8JD7uW_bottomPanel,._8JD7uW_bottomPanelHidden,._8JD7uW_toggleCluster,._8JD7uW_toggleButton,._8JD7uW_tab,._8JD7uW_tabBarPlus,._8JD7uW_paneCard,._8JD7uW_explorerRow,._8JD7uW_gitRow,._8JD7uW_divider,._8JD7uW_dividerRow:after,._8JD7uW_dividerCol:after{transition:none;animation:none}}@media (width<=767px){._8JD7uW_panel:not(._8JD7uW_panelHidden) ._8JD7uW_tabBar{padding-right:40px}._8JD7uW_tab{min-width:48px;max-width:128px}}";
		const tagId$4 = "dsh-external/@aphroditee666/dsh-best-sidebar/sidebar.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId$4) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-external/@aphroditee666/dsh-best-sidebar";
			tag.dataset.pluginCss = tagId$4;
			tag.textContent = css$4;
			document.head.appendChild(tag);
		}
		var sidebar_module_css_default = {
			"editorMd": "_8JD7uW_editorMd",
			"editorPdf": "_8JD7uW_editorPdf",
			"editorTreeToggleActive": "_8JD7uW_editorTreeToggleActive",
			"gitDiffFilePath": "_8JD7uW_gitDiffFilePath",
			"bottomResizeActive": "_8JD7uW_bottomResizeActive",
			"mermaidModalButton": "_8JD7uW_mermaidModalButton",
			"selectionPopup": "_8JD7uW_selectionPopup",
			"terminalDepsHint": "_8JD7uW_terminalDepsHint",
			"dividerRow": "_8JD7uW_dividerRow",
			"gitHeader": "_8JD7uW_gitHeader",
			"gitSection": "_8JD7uW_gitSection",
			"editorDownloadLink": "_8JD7uW_editorDownloadLink",
			"gitRow": "_8JD7uW_gitRow",
			"editorTreePanel": "_8JD7uW_editorTreePanel",
			"gitConfirmDesc": "_8JD7uW_gitConfirmDesc",
			"explorerSymlink": "_8JD7uW_explorerSymlink",
			"bottomClose": "_8JD7uW_bottomClose",
			"uploadOverlayCancel": "_8JD7uW_uploadOverlayCancel",
			"gitDiffTabTitle": "_8JD7uW_gitDiffTabTitle",
			"gitBadge": "_8JD7uW_gitBadge",
			"paneEmptyCards": "_8JD7uW_paneEmptyCards",
			"gitCommitButton": "_8JD7uW_gitCommitButton",
			"gitLogHash": "_8JD7uW_gitLogHash",
			"toggleCluster": "_8JD7uW_toggleCluster",
			"terminal": "_8JD7uW_terminal",
			"gitDiffMeta": "_8JD7uW_gitDiffMeta",
			"uploadDropChatCard": "_8JD7uW_uploadDropChatCard",
			"tabBar": "_8JD7uW_tabBar",
			"tabActive": "_8JD7uW_tabActive",
			"browser": "_8JD7uW_browser",
			"dropDown": "_8JD7uW_dropDown",
			"panelHidden": "_8JD7uW_panelHidden",
			"explorerRow": "_8JD7uW_explorerRow",
			"editorHeader": "_8JD7uW_editorHeader",
			"gitDiffAdd": "_8JD7uW_gitDiffAdd",
			"browserBlockedActions": "_8JD7uW_browserBlockedActions",
			"producedRow": "_8JD7uW_producedRow",
			"editorDocxZoomRange": "_8JD7uW_editorDocxZoomRange",
			"mermaidHeader": "_8JD7uW_mermaidHeader",
			"editorTreeSearch": "_8JD7uW_editorTreeSearch",
			"browserBlockedDesc": "_8JD7uW_browserBlockedDesc",
			"mermaidModalStage": "_8JD7uW_mermaidModalStage",
			"browserStart": "_8JD7uW_browserStart",
			"editorCmHidden": "_8JD7uW_editorCmHidden",
			"editorPdfToolbar": "_8JD7uW_editorPdfToolbar",
			"terminalWrap": "_8JD7uW_terminalWrap",
			"gitRowMain": "_8JD7uW_gitRowMain",
			"gitDiffExpand": "_8JD7uW_gitDiffExpand",
			"gitLogMore": "_8JD7uW_gitLogMore",
			"terminalDepsNote": "_8JD7uW_terminalDepsNote",
			"producedChip": "_8JD7uW_producedChip",
			"paneTab": "_8JD7uW_paneTab",
			"gitBranchSelect": "_8JD7uW_gitBranchSelect",
			"bottomPanelHidden": "_8JD7uW_bottomPanelHidden",
			"gitName": "_8JD7uW_gitName",
			"gitLogSubject": "_8JD7uW_gitLogSubject",
			"explorerRoot": "_8JD7uW_explorerRoot",
			"dsh-row-in": "_8JD7uW_dsh-row-in",
			"uploadOverlayProgress": "_8JD7uW_uploadOverlayProgress",
			"bottomResize": "_8JD7uW_bottomResize",
			"uploadDropHero": "_8JD7uW_uploadDropHero",
			"mermaidError": "_8JD7uW_mermaidError",
			"gitDiff": "_8JD7uW_gitDiff",
			"gitCommitInput": "_8JD7uW_gitCommitInput",
			"gitLogLine1": "_8JD7uW_gitLogLine1",
			"uploadOverlay": "_8JD7uW_uploadOverlay",
			"gitSectionHeader": "_8JD7uW_gitSectionHeader",
			"gitLogRef": "_8JD7uW_gitLogRef",
			"gitLogMeta": "_8JD7uW_gitLogMeta",
			"gitLogLine2": "_8JD7uW_gitLogLine2",
			"editorPathInput": "_8JD7uW_editorPathInput",
			"mermaidWrap": "_8JD7uW_mermaidWrap",
			"terminalDepsTitle": "_8JD7uW_terminalDepsTitle",
			"terminalBannerUrl": "_8JD7uW_terminalBannerUrl",
			"explorerError": "_8JD7uW_explorerError",
			"uploadOverlayStatus": "_8JD7uW_uploadOverlayStatus",
			"editorPdfDragShieldActive": "_8JD7uW_editorPdfDragShieldActive",
			"tabBadge": "_8JD7uW_tabBadge",
			"terminalDepsCommandRow": "_8JD7uW_terminalDepsCommandRow",
			"uploadDropChatHint": "_8JD7uW_uploadDropChatHint",
			"sandboxStatusText": "_8JD7uW_sandboxStatusText",
			"producedMore": "_8JD7uW_producedMore",
			"dropUp": "_8JD7uW_dropUp",
			"uploadOverlayTitle": "_8JD7uW_uploadOverlayTitle",
			"paneTabHidden": "_8JD7uW_paneTabHidden",
			"editorImageWrap": "_8JD7uW_editorImageWrap",
			"explorerCopied": "_8JD7uW_explorerCopied",
			"editorHtml": "_8JD7uW_editorHtml",
			"panelResizeActive": "_8JD7uW_panelResizeActive",
			"tabTitle": "_8JD7uW_tabTitle",
			"dropCenter": "_8JD7uW_dropCenter",
			"editor": "_8JD7uW_editor",
			"editorBinary": "_8JD7uW_editorBinary",
			"sandboxStatus": "_8JD7uW_sandboxStatus",
			"mermaidCopy": "_8JD7uW_mermaidCopy",
			"editorPdfFrame": "_8JD7uW_editorPdfFrame",
			"editorModeToggle": "_8JD7uW_editorModeToggle",
			"mermaidModalToolbar": "_8JD7uW_mermaidModalToolbar",
			"gitDiffFileTag": "_8JD7uW_gitDiffFileTag",
			"gitDiffDel": "_8JD7uW_gitDiffDel",
			"gitEmpty": "_8JD7uW_gitEmpty",
			"paneContent": "_8JD7uW_paneContent",
			"gitDiffCtx": "_8JD7uW_gitDiffCtx",
			"uploadDropZone": "_8JD7uW_uploadDropZone",
			"editorBanner": "_8JD7uW_editorBanner",
			"gitRowSelected": "_8JD7uW_gitRowSelected",
			"explorerHidden": "_8JD7uW_explorerHidden",
			"gitDiffHunkSection": "_8JD7uW_gitDiffHunkSection",
			"editorStatus": "_8JD7uW_editorStatus",
			"editorStatusError": "_8JD7uW_editorStatusError",
			"editorPdfFrameBlocked": "_8JD7uW_editorPdfFrameBlocked",
			"paneDrop": "_8JD7uW_paneDrop",
			"tabBarPlus": "_8JD7uW_tabBarPlus",
			"editorPdfDragShield": "_8JD7uW_editorPdfDragShield",
			"editorModeButton": "_8JD7uW_editorModeButton",
			"terminalBanner": "_8JD7uW_terminalBanner",
			"explorerBody": "_8JD7uW_explorerBody",
			"uploadDropZoneText": "_8JD7uW_uploadDropZoneText",
			"editorSearchHint": "_8JD7uW_editorSearchHint",
			"editorBody": "_8JD7uW_editorBody",
			"pane": "_8JD7uW_pane",
			"explorerRowDropTarget": "_8JD7uW_explorerRowDropTarget",
			"uploadOverlayCard": "_8JD7uW_uploadOverlayCard",
			"terminalRetry": "_8JD7uW_terminalRetry",
			"gitCommit": "_8JD7uW_gitCommit",
			"tabList": "_8JD7uW_tabList",
			"gitLogRow": "_8JD7uW_gitLogRow",
			"mermaidCode": "_8JD7uW_mermaidCode",
			"uploadDropZonePill": "_8JD7uW_uploadDropZonePill",
			"editorError": "_8JD7uW_editorError",
			"explorerName": "_8JD7uW_explorerName",
			"explorerBroken": "_8JD7uW_explorerBroken",
			"gitDiffFile": "_8JD7uW_gitDiffFile",
			"panelBody": "_8JD7uW_panelBody",
			"gitDiffFileOld": "_8JD7uW_gitDiffFileOld",
			"workbench": "_8JD7uW_workbench",
			"editorTreeResize": "_8JD7uW_editorTreeResize",
			"gitDiffHunkHeader": "_8JD7uW_gitDiffHunkHeader",
			"mermaidModalHint": "_8JD7uW_mermaidModalHint",
			"mermaidMarkdown": "_8JD7uW_mermaidMarkdown",
			"dropRight": "_8JD7uW_dropRight",
			"gitDiffLine": "_8JD7uW_gitDiffLine",
			"gitDiffMetaText": "_8JD7uW_gitDiffMetaText",
			"explorer": "_8JD7uW_explorer",
			"browserInput": "_8JD7uW_browserInput",
			"gitDiffTab": "_8JD7uW_gitDiffTab",
			"gitDiffTabHeader": "_8JD7uW_gitDiffTabHeader",
			"gitDiffNum": "_8JD7uW_gitDiffNum",
			"explorerEmpty": "_8JD7uW_explorerEmpty",
			"orphanedType": "_8JD7uW_orphanedType",
			"divider": "_8JD7uW_divider",
			"editorSearchInput": "_8JD7uW_editorSearchInput",
			"editorTreePanelFull": "_8JD7uW_editorTreePanelFull",
			"editorPlaceholder": "_8JD7uW_editorPlaceholder",
			"browserFrame": "_8JD7uW_browserFrame",
			"mermaidInfo": "_8JD7uW_mermaidInfo",
			"mermaidBody": "_8JD7uW_mermaidBody",
			"sandboxAction": "_8JD7uW_sandboxAction",
			"editorPdfStage": "_8JD7uW_editorPdfStage",
			"explorerRef": "_8JD7uW_explorerRef",
			"terminalDepsActions": "_8JD7uW_terminalDepsActions",
			"iconButton": "_8JD7uW_iconButton",
			"git": "_8JD7uW_git",
			"splitRow": "_8JD7uW_splitRow",
			"dropOverlay": "_8JD7uW_dropOverlay",
			"sandboxStatusOff": "_8JD7uW_sandboxStatusOff",
			"editorImage": "_8JD7uW_editorImage",
			"mermaidModal": "_8JD7uW_mermaidModal",
			"split": "_8JD7uW_split",
			"splitCol": "_8JD7uW_splitCol",
			"gitError": "_8JD7uW_gitError",
			"toggleButton": "_8JD7uW_toggleButton",
			"bottomPanel": "_8JD7uW_bottomPanel",
			"browserBar": "_8JD7uW_browserBar",
			"gitPlaceholder": "_8JD7uW_gitPlaceholder",
			"producedLabel": "_8JD7uW_producedLabel",
			"uploadOverlayProgressFill": "_8JD7uW_uploadOverlayProgressFill",
			"editorBinaryNotice": "_8JD7uW_editorBinaryNotice",
			"sandboxDot": "_8JD7uW_sandboxDot",
			"editorTitle": "_8JD7uW_editorTitle",
			"browserBlockedTitle": "_8JD7uW_browserBlockedTitle",
			"editorCm": "_8JD7uW_editorCm",
			"editorSearchResult": "_8JD7uW_editorSearchResult",
			"boundaryError": "_8JD7uW_boundaryError",
			"dropLeft": "_8JD7uW_dropLeft",
			"gitDiffHunk": "_8JD7uW_gitDiffHunk",
			"tab": "_8JD7uW_tab",
			"panelResize": "_8JD7uW_panelResize",
			"sandboxStatusOn": "_8JD7uW_sandboxStatusOn",
			"terminalRepairCommand": "_8JD7uW_terminalRepairCommand",
			"panel": "_8JD7uW_panel",
			"explorerHeader": "_8JD7uW_explorerHeader",
			"gitDiffCode": "_8JD7uW_gitDiffCode",
			"cornerHandle": "_8JD7uW_cornerHandle",
			"explorerDir": "_8JD7uW_explorerDir",
			"browserBlocked": "_8JD7uW_browserBlocked",
			"splitChild": "_8JD7uW_splitChild",
			"paneCard": "_8JD7uW_paneCard",
			"editorMain": "_8JD7uW_editorMain",
			"editorTreeDock": "_8JD7uW_editorTreeDock",
			"browserMessage": "_8JD7uW_browserMessage",
			"dividerActive": "_8JD7uW_dividerActive",
			"dividerCol": "_8JD7uW_dividerCol",
			"tabClose": "_8JD7uW_tabClose",
			"browserBlockedButton": "_8JD7uW_browserBlockedButton",
			"editorModeActive": "_8JD7uW_editorModeActive",
			"terminalDepsBanner": "_8JD7uW_terminalDepsBanner",
			"tabBoundaryError": "_8JD7uW_tabBoundaryError",
			"tabBarDrop": "_8JD7uW_tabBarDrop",
			"dirtyDot": "_8JD7uW_dirtyDot",
			"editorPptxButton": "_8JD7uW_editorPptxButton",
			"gitLink": "_8JD7uW_gitLink"
		};
		//#endregion
		...
		*/
		...
		const api = {
			...
			*  side panel's search box); matches are cwd-relative '/'-separated paths. */
			...
			fsCreateDir: (scope, path) => call("fs.createDirectory", scopePayload(scope, { path })),
			/** Reveal a file/folder in the OS file manager; wire method `fs.reveal`. */
			fsReveal: (scope, path) => call("fs.reveal", scopePayload(scope, { path })),
		...
		*/
		function TreePanel(props) {
			...
			*  summarizing the partial results. */
			...
			const autoExpandedRef = (0, react.useRef)(expanded);
			autoExpandedRef.current = expanded;
			const autoSigRef = (0, react.useRef)("");
			(0, react.useEffect)(() => {
				const timer = setInterval(async () => {
					if (document.visibilityState !== "visible") return;
					try {
						const dirs = [cwd, ...autoExpandedRef.current].filter((d) => d !== void 0);
						const sig = JSON.stringify(await Promise.all(dirs.map(async (d) => {
							try {
								const names = ((await api.fsTree({
									sessionId,
									cwd
								}, d)).entries ?? []).map((e) => e.path).sort();
								return d + "=" + names.join(",");
							} catch {
								return d + "=ERR";
							}
			...
			(0, react.useEffect)(() => {
				...
				loadChunk(chunk).then((mod) => {
					...
					if (Comp === void 0) {
						setState({
							status: "error",
							message: `[dsh-best-sidebar] chunk "${chunk}" is missing its component`
						});
						return;
					}
					setState({
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:D:\BaiduSyncdisk\obsidian\Github\clyde_dsh\dsh_projects\plugins\dsh-best-sidebar\src\client\SubagentView.module.css.mjs
		const css$3 = ".Pg7T8W_subagent{flex-direction:column;flex:1;min-height:0;display:flex}.Pg7T8W_subagentHeader{flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.Pg7T8W_subagentTitle{min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.Pg7T8W_subagentCount{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.Pg7T8W_subagentRefresh{width:24px;height:24px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;justify-content:center;align-items:center;display:inline-flex}.Pg7T8W_subagentRefresh:hover{background:var(--dsw-alias-interactive-bg-hover)}.Pg7T8W_subagentBody{flex:1;min-height:0;padding:2px 6px 8px;overflow-y:auto}.Pg7T8W_subagentRow{box-sizing:border-box;width:100%;min-height:50px;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;background:0 0;border:none;border-radius:8px;outline:none;align-items:flex-start;gap:8px;padding:7px 8px 7px 11px;display:flex;position:relative}.Pg7T8W_subagentRow:hover,.Pg7T8W_subagentRow:focus-visible{background:var(--dsw-alias-interactive-bg-hover)}.Pg7T8W_subagentRowActive,.Pg7T8W_subagentRowActive:hover,.Pg7T8W_subagentRowActive:focus-visible{background:var(--dsw-alias-interactive-bg-active)}.Pg7T8W_subagentRowDisabled{color:var(--dsw-alias-label-dimmed);cursor:not-allowed}.Pg7T8W_subagentRowDisabled:hover{background:0 0}.Pg7T8W_subagentRowLoading{cursor:default}.Pg7T8W_subagentDot{margin-top:4px}.Pg7T8W_subagentContent{flex-direction:column;flex:1;gap:2px;min-width:0;display:flex}.Pg7T8W_subagentLabel,.Pg7T8W_subagentSecondary{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.Pg7T8W_subagentLabel{color:inherit;font-weight:400}.Pg7T8W_subagentSecondary{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}.Pg7T8W_subagentLive{min-width:0;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);align-items:baseline;gap:4px;display:flex;overflow:hidden}.Pg7T8W_subagentLiveTool{font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);flex:none}.Pg7T8W_subagentLiveArgs{min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.Pg7T8W_subagentLiveText{-webkit-line-clamp:2;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-secondary);-webkit-box-orient:vertical;display:-webkit-box;overflow:hidden}.Pg7T8W_subagentNode{min-width:0;position:relative}.Pg7T8W_subagentChildren{margin-left:18px;padding-left:4px;position:relative}.Pg7T8W_subagentChildren:before{content:\"\";border-left:1px solid var(--dsw-alias-border-l2);height:26px;position:absolute;top:-26px;left:0}.Pg7T8W_subagentChildren[aria-busy=true]:before{content:none}.Pg7T8W_subagentChildren>.Pg7T8W_subagentNode:before{content:\"\";border-left:1px solid var(--dsw-alias-border-l2);position:absolute;top:0;bottom:0;left:-4px}.Pg7T8W_subagentChildren>.Pg7T8W_subagentNode:last-child:before{height:17px;bottom:auto}.Pg7T8W_subagentChildren>.Pg7T8W_subagentNode>.Pg7T8W_subagentRow:before{content:\"\";border-top:1px solid var(--dsw-alias-border-l2);width:14px;position:absolute;top:16px;left:-4px}.Pg7T8W_subagentEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;flex-direction:column;gap:2px;padding:16px;display:flex}.Pg7T8W_subagentEmptyHint{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-dimmed)}.Pg7T8W_subagentError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);justify-content:space-between;align-items:center;gap:8px;padding:8px 10px;display:flex}.Pg7T8W_subagentErrorRetry{height:24px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;align-items:center;gap:4px;padding:0 8px;display:inline-flex}.Pg7T8W_subagentErrorRetry:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.Pg7T8W_jobs{border-top:1px solid var(--dsw-alias-border-l2);margin-top:10px;padding-top:8px}.Pg7T8W_jobsHeader{align-items:center;gap:8px;height:26px;padding:0 2px;display:flex}.Pg7T8W_jobsTitle{min-width:0;font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.Pg7T8W_jobsCount{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.Pg7T8W_jobsList{flex-direction:column;gap:2px;margin:0;padding:0;list-style:none;display:flex}.Pg7T8W_jobsRow{border-radius:8px;align-items:center;gap:4px;display:flex}.Pg7T8W_jobsRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.Pg7T8W_jobsRowSettled{opacity:.8}.Pg7T8W_jobsRowSelected,.Pg7T8W_jobsRowSelected:hover{background:var(--dsw-alias-interactive-bg-active)}.Pg7T8W_jobsRowMain{min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;background:0 0;border:none;border-radius:8px;outline:none;flex:1;align-items:flex-start;gap:8px;padding:6px 8px 6px 11px;display:flex}.Pg7T8W_jobsRowMain:focus-visible{background:var(--dsw-alias-interactive-bg-hover)}.Pg7T8W_jobsDot{margin-top:5px}.Pg7T8W_jobsContent{flex-direction:column;gap:1px;min-width:0;display:flex}.Pg7T8W_jobsLabelLine{align-items:center;gap:6px;min-width:0;display:flex}.Pg7T8W_jobsKind{text-overflow:ellipsis;white-space:nowrap;border:1px solid var(--dsw-alias-border-l2);max-width:90px;font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-tertiary);border-radius:4px;flex:none;padding:0 5px;line-height:14px;overflow:hidden}.Pg7T8W_jobsLabel{text-overflow:ellipsis;white-space:nowrap;min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.Pg7T8W_jobsSecondary{text-overflow:ellipsis;white-space:nowrap;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);overflow:hidden}.Pg7T8W_jobsKill{width:22px;height:22px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;justify-content:center;align-items:center;margin-right:4px;display:inline-flex}.Pg7T8W_jobsKill:hover{background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent);color:var(--dsw-alias-state-error-primary)}.Pg7T8W_jobsKillArmed,.Pg7T8W_jobsKillArmed:hover{background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent);width:auto;height:20px;color:var(--dsw-alias-state-error-primary);font:var(--dsw-font-xxxs-strong-11);white-space:nowrap;padding:0 8px}.Pg7T8W_jobsKill:disabled{opacity:.5;cursor:default}.Pg7T8W_jobsKillError{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-error-primary);flex:none;margin-right:4px}.Pg7T8W_jobsPane{z-index:1;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);border-radius:8px;margin-top:4px;position:sticky;bottom:0;overflow:hidden;box-shadow:0 -6px 12px -8px #00000059}.Pg7T8W_jobsPaneHeader{border-bottom:1px solid var(--dsw-alias-border-l1);align-items:center;gap:6px;height:28px;padding:0 4px 0 10px;display:flex}.Pg7T8W_jobsPaneDot{flex:none}.Pg7T8W_jobsPaneLabel{text-overflow:ellipsis;white-space:nowrap;min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.Pg7T8W_jobsPaneStatus{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.Pg7T8W_jobsPaneClose{width:20px;height:20px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:5px;flex:none;justify-content:center;align-items:center;display:inline-flex}.Pg7T8W_jobsPaneClose:hover{background:var(--dsw-alias-interactive-bg-hover)}.Pg7T8W_jobsPanePre{max-height:200px;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);color:var(--dsw-alias-label-primary);white-space:pre-wrap;word-break:break-word;margin:0;padding:6px 10px;line-height:1.5;overflow:auto}.Pg7T8W_jobsPaneHint{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);padding:8px 10px}.Pg7T8W_jobsPaneError{color:var(--dsw-alias-state-error-primary)}";
		const tagId$3 = "dsh-external/@aphroditee666/dsh-best-sidebar/SubagentView.module.css";
		...
		var SubagentView_module_css_default = {
			"jobsDot": "Pg7T8W_jobsDot",
			"jobsLabelLine": "Pg7T8W_jobsLabelLine",
			"subagentEmpty": "Pg7T8W_subagentEmpty",
			"jobsKillError": "Pg7T8W_jobsKillError",
			"jobsKillArmed": "Pg7T8W_jobsKillArmed",
			"subagentEmptyHint": "Pg7T8W_subagentEmptyHint",
			"subagentRefresh": "Pg7T8W_subagentRefresh",
			"jobsHeader": "Pg7T8W_jobsHeader",
			"jobsPane": "Pg7T8W_jobsPane",
			"subagentLiveText": "Pg7T8W_subagentLiveText",
			"jobsList": "Pg7T8W_jobsList",
			"jobsPaneDot": "Pg7T8W_jobsPaneDot",
			"subagentRow": "Pg7T8W_subagentRow",
			"jobs": "Pg7T8W_jobs",
			"jobsRowSelected": "Pg7T8W_jobsRowSelected",
			"subagentCount": "Pg7T8W_subagentCount",
			"jobsSecondary": "Pg7T8W_jobsSecondary",
			"jobsKind": "Pg7T8W_jobsKind",
			"subagentLiveTool": "Pg7T8W_subagentLiveTool",
			"jobsRowMain": "Pg7T8W_jobsRowMain",
			"jobsPaneClose": "Pg7T8W_jobsPaneClose",
			"jobsCount": "Pg7T8W_jobsCount",
			"subagentContent": "Pg7T8W_subagentContent",
			"subagentHeader": "Pg7T8W_subagentHeader",
			"subagentTitle": "Pg7T8W_subagentTitle",
			"subagentDot": "Pg7T8W_subagentDot",
			"jobsContent": "Pg7T8W_jobsContent",
			"subagentBody": "Pg7T8W_subagentBody",
			"subagentLabel": "Pg7T8W_subagentLabel",
			"subagentNode": "Pg7T8W_subagentNode",
			"subagentRowDisabled": "Pg7T8W_subagentRowDisabled",
			"subagentLive": "Pg7T8W_subagentLive",
			"jobsTitle": "Pg7T8W_jobsTitle",
			"jobsKill": "Pg7T8W_jobsKill",
			"subagentErrorRetry": "Pg7T8W_subagentErrorRetry",
			"subagentRowActive": "Pg7T8W_subagentRowActive",
			"subagentSecondary": "Pg7T8W_subagentSecondary",
			"jobsPaneHeader": "Pg7T8W_jobsPaneHeader",
			"jobsPaneLabel": "Pg7T8W_jobsPaneLabel",
			"jobsPanePre": "Pg7T8W_jobsPanePre",
			"jobsPaneHint": "Pg7T8W_jobsPaneHint",
			"jobsPaneError": "Pg7T8W_jobsPaneError",
			"subagentRowLoading": "Pg7T8W_subagentRowLoading",
			"subagentLiveArgs": "Pg7T8W_subagentLiveArgs",
			"jobsRowSettled": "Pg7T8W_jobsRowSettled",
			"jobsPaneStatus": "Pg7T8W_jobsPaneStatus",
			"jobsLabel": "Pg7T8W_jobsLabel",
			"jobsRow": "Pg7T8W_jobsRow",
			"subagent": "Pg7T8W_subagent",
			"subagentError": "Pg7T8W_subagentError",
			"subagentChildren": "Pg7T8W_subagentChildren"
		};
		//#endregion
		...
		*/
			...
			for (let index = 0; index < events.length; index++) {
				...
				switch (event.type) {
					case "user/message": {
						...
						if (text.startsWith("Side conversation boundary")) break;
						rows.push({
							kind: "user",
							seq: event.seq,
							text
		...
		//#endregion
		//#region \0dsh-css:D:\BaiduSyncdisk\obsidian\Github\clyde_dsh\dsh_projects\plugins\dsh-best-sidebar\src\client\SideChatView.module.css.mjs
		const css$2 = ".LCL6ZG_sidechat{flex-direction:column;flex:1;min-height:0;display:flex}.LCL6ZG_sidechatDetailHeader{border-bottom:1px solid var(--dsw-alias-hairline);flex:none;align-items:center;gap:4px;min-height:36px;padding:4px 8px 4px 12px;display:flex}.LCL6ZG_sidechatHeaderDot{flex:none}.LCL6ZG_sidechatHeaderSpacer{flex:1;min-width:0}.LCL6ZG_sidechatAgentBadge{border:1px solid var(--dsw-alias-hairline);max-width:55%;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;border-radius:999px;flex:none;padding:1px 8px;overflow:hidden}.LCL6ZG_sidechatIconBtn{width:26px;height:26px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;justify-content:center;align-items:center;padding:0;transition:background-color .1s ease-out,color .1s ease-out;display:inline-flex}.LCL6ZG_sidechatIconBtn:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.LCL6ZG_sidechatIconBtn:disabled{opacity:.4;cursor:default}.LCL6ZG_sidechatHero{min-height:0;color:var(--dsw-alias-label-tertiary);text-align:center;flex-direction:column;flex:1;justify-content:center;align-items:center;gap:8px;padding:24px 20px;animation:.2s ease-out LCL6ZG_sidechatFadeIn;display:flex}.LCL6ZG_sidechatHeroTitle{font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);font-weight:500}.LCL6ZG_sidechatHeroDesc{max-width:300px;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);line-height:1.6}.LCL6ZG_sidechatPrimaryBtn{background:var(--dsw-alias-button-info-fill,var(--dsw-alias-accent));color:var(--dsw-alias-button-info-label,var(--dsw-alias-accent-ink,#fff));font:var(--dsw-font-s-13);cursor:pointer;border:none;border-radius:999px;flex:none;margin-top:4px;padding:6px 14px;transition:opacity .1s ease-out}.LCL6ZG_sidechatPrimaryBtn:hover:not(:disabled){opacity:.88}.LCL6ZG_sidechatPrimaryBtn:disabled{opacity:.4;cursor:default}.LCL6ZG_sidechatHint{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);flex:none;padding:4px 12px}.LCL6ZG_sidechatError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-danger);flex:none;padding:4px 12px}.LCL6ZG_sidechatScroll{flex-direction:column;flex:1;gap:10px;min-height:0;padding:10px 12px;display:flex;overflow-y:auto}.LCL6ZG_sidechatScroll>*{animation:.18s ease-out LCL6ZG_sidechatRowIn}.LCL6ZG_sidechatUser{background:var(--dsw-specific-bubble,var(--dsw-alias-bg-base));max-width:88%;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);overflow-wrap:anywhere;border-radius:18px;align-self:flex-end;padding:8px 14px}.LCL6ZG_sidechatAssistant{font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);overflow-wrap:anywhere;align-self:stretch}.LCL6ZG_sidechatReasoning{align-self:stretch}.LCL6ZG_sidechatReasoningSummary{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);cursor:pointer;list-style:none;display:inline-block}.LCL6ZG_sidechatReasoningSummary::-webkit-details-marker{display:none}.LCL6ZG_sidechatReasoningBody{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);white-space:pre-wrap;overflow-wrap:anywhere;margin-top:2px;padding-left:2px}.LCL6ZG_sidechatTool{border:1px solid var(--dsw-alias-hairline);background:var(--dsw-alias-bg-base);border-radius:10px;align-self:stretch;overflow:hidden}.LCL6ZG_sidechatToolSummary{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-secondary);cursor:pointer;align-items:center;gap:6px;padding:5px 10px;list-style:none;display:flex}.LCL6ZG_sidechatToolSummary::-webkit-details-marker{display:none}.LCL6ZG_sidechatToolName{font-family:var(--dsw-font-mono);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.LCL6ZG_sidechatToolExecuting{background-image:linear-gradient(90deg, transparent 0%, var(--dsw-alias-interactive-bg-hover) 50%, transparent 100%);background-size:200% 100%;animation:2.6s linear infinite LCL6ZG_sidechatSweep}.LCL6ZG_sidechatToolFailed{color:var(--dsw-alias-danger)}.LCL6ZG_sidechatToolArgs,.LCL6ZG_sidechatToolResult{border-top:1px solid var(--dsw-alias-hairline);font:var(--dsw-font-xxs-12);font-family:var(--dsw-font-mono);color:var(--dsw-alias-label-secondary);white-space:pre-wrap;overflow-wrap:anywhere;max-height:220px;margin:0;padding:6px 10px;overflow-y:auto}.LCL6ZG_sidechatStatus{flex:none;align-items:center;gap:8px;padding:2px 14px 6px;animation:.16s ease-out LCL6ZG_sidechatFadeIn;display:flex}.LCL6ZG_sidechatStatusText{font:var(--dsw-font-xxs-12);background-image:linear-gradient(90deg, var(--dsw-alias-label-tertiary) 0%, var(--dsw-alias-label-primary) 50%, var(--dsw-alias-label-tertiary) 100%);color:#0000;background-size:200% 100%;-webkit-background-clip:text;background-clip:text;animation:2.6s linear infinite LCL6ZG_sidechatSweep}.LCL6ZG_sidechatComposer{border:1px solid var(--dsw-alias-border-l2-darkmode-thin,var(--dsw-alias-hairline));background:var(--dsw-specific-input-major,var(--dsw-alias-bg-base));box-shadow:var(--dsw-shadow-lv2,none);border-radius:16px;flex-direction:column;flex:none;gap:4px;margin:0 8px 8px;padding:8px 8px 6px 14px;display:flex}.LCL6ZG_sidechatComposerInput{box-sizing:border-box;width:100%;color:var(--dsw-alias-label-primary);font:var(--dsw-font-s-14);resize:none;background:0 0;border:none;outline:none;max-height:132px;padding:2px 0;line-height:22px}.LCL6ZG_sidechatComposerInput::placeholder{color:var(--dsw-alias-label-tertiary)}.LCL6ZG_sidechatComposerBar{flex:none;align-items:center;gap:8px;min-height:28px;display:flex}.LCL6ZG_sidechatComposerMeta{min-width:0;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.LCL6ZG_sidechatSendBtn{background:var(--dsw-alias-button-info-fill,var(--dsw-alias-accent));width:28px;height:28px;color:var(--dsw-alias-button-info-label,var(--dsw-alias-accent-ink,#fff));cursor:pointer;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;transition:opacity .1s ease-out;animation:.12s ease-out LCL6ZG_sidechatBtnIn;display:inline-flex}.LCL6ZG_sidechatSendBtn:hover:not(:disabled){opacity:.88}.LCL6ZG_sidechatSendBtn:disabled{opacity:.35;cursor:default}@keyframes LCL6ZG_sidechatRowIn{0%{opacity:0;transform:translateY(4px)}to{opacity:1;transform:translateY(0)}}@keyframes LCL6ZG_sidechatFadeIn{0%{opacity:0}to{opacity:1}}@keyframes LCL6ZG_sidechatBtnIn{0%{opacity:0;transform:scale(.85)}to{opacity:1;transform:scale(1)}}@keyframes LCL6ZG_sidechatSweep{0%{background-position:200% 0}to{background-position:-200% 0}}@media (prefers-reduced-motion:reduce){.LCL6ZG_sidechatScroll>*,.LCL6ZG_sidechatHero,.LCL6ZG_sidechatStatus,.LCL6ZG_sidechatSendBtn{animation:none}.LCL6ZG_sidechatStatusText,.LCL6ZG_sidechatToolExecuting{color:var(--dsw-alias-label-tertiary);background-image:none;animation:none}}";
		const tagId$2 = "dsh-external/@aphroditee666/dsh-best-sidebar/SideChatView.module.css";
		...
		var SideChatView_module_css_default = {
			"sidechatIconBtn": "LCL6ZG_sidechatIconBtn",
			"sidechatComposerInput": "LCL6ZG_sidechatComposerInput",
			"sidechatTool": "LCL6ZG_sidechatTool",
			"sidechatUser": "LCL6ZG_sidechatUser",
			"sidechatReasoningSummary": "LCL6ZG_sidechatReasoningSummary",
			"sidechatStatus": "LCL6ZG_sidechatStatus",
			"sidechatRowIn": "LCL6ZG_sidechatRowIn",
			"sidechatHint": "LCL6ZG_sidechatHint",
			"sidechatToolFailed": "LCL6ZG_sidechatToolFailed",
			"sidechatStatusText": "LCL6ZG_sidechatStatusText",
			"sidechatComposer": "LCL6ZG_sidechatComposer",
			"sidechatComposerMeta": "LCL6ZG_sidechatComposerMeta",
			"sidechatToolArgs": "LCL6ZG_sidechatToolArgs",
			"sidechat": "LCL6ZG_sidechat",
			"sidechatHero": "LCL6ZG_sidechatHero",
			"sidechatHeroDesc": "LCL6ZG_sidechatHeroDesc",
			"sidechatToolExecuting": "LCL6ZG_sidechatToolExecuting",
			"sidechatComposerBar": "LCL6ZG_sidechatComposerBar",
			"sidechatAgentBadge": "LCL6ZG_sidechatAgentBadge",
			"sidechatHeroTitle": "LCL6ZG_sidechatHeroTitle",
			"sidechatSweep": "LCL6ZG_sidechatSweep",
			"sidechatToolResult": "LCL6ZG_sidechatToolResult",
			"sidechatHeaderSpacer": "LCL6ZG_sidechatHeaderSpacer",
			"sidechatBtnIn": "LCL6ZG_sidechatBtnIn",
			"sidechatAssistant": "LCL6ZG_sidechatAssistant",
			"sidechatPrimaryBtn": "LCL6ZG_sidechatPrimaryBtn",
			"sidechatReasoning": "LCL6ZG_sidechatReasoning",
			"sidechatToolSummary": "LCL6ZG_sidechatToolSummary",
			"sidechatSendBtn": "LCL6ZG_sidechatSendBtn",
			"sidechatError": "LCL6ZG_sidechatError",
			"sidechatHeaderDot": "LCL6ZG_sidechatHeaderDot",
			"sidechatToolName": "LCL6ZG_sidechatToolName",
			"sidechatFadeIn": "LCL6ZG_sidechatFadeIn",
			"sidechatReasoningBody": "LCL6ZG_sidechatReasoningBody",
			"sidechatScroll": "LCL6ZG_sidechatScroll",
			"sidechatDetailHeader": "LCL6ZG_sidechatDetailHeader"
		};
		//#endregion
		...
		function renderRow(row, labels) {
			switch (row.kind) {
				...
				case "reasoning": return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
					className: SideChatView_module_css_default.sidechatReasoning,
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("summary", {
						className: SideChatView_module_css_default.sidechatReasoningSummary,
						children: labels.thinkLabel
					}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: SideChatView_module_css_default.sidechatReasoningBody,
						children: row.text
					})]
				}, row.seq);
				case "tool": return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
					className: SideChatView_module_css_default.sidechatTool,
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("summary", {
							className: clsx(SideChatView_module_css_default.sidechatToolSummary, row.executing === true && SideChatView_module_css_default.sidechatToolExecuting, row.failed && SideChatView_module_css_default.sidechatToolFailed),
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.StateDot, {
								state: row.executing === true ? "ongoing" : row.failed ? "error" : "done",
								size: 8
							}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: SideChatView_module_css_default.sidechatToolName,
								children: row.name
							})]
						}),
						row.args !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("pre", {
							className: SideChatView_module_css_default.sidechatToolArgs,
							children: row.args
						}),
						row.resultText !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("pre", {
							className: SideChatView_module_css_default.sidechatToolResult,
			...
			if (threadId === void 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: SideChatView_module_css_default.sidechatHeroTitle,
		...
		*/
		function appendToDraft(ctx, sessionId, text) {
			try {
				...
				console.warn("[dsh-best-sidebar] draft insert failed:", error);
				return false;
			}
		}
		//#endregion
		...
		*/
		var RenderBoundary = class extends react.Component {
			...
			componentDidCatch(error, info) {
				console.error("[dsh-best-sidebar] render error:", error, info.componentStack);
			}
			render() {
				if (this.state.error !== null) return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: this.props.className,
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: ["dsh-best-sidebar: ", this.state.error] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						type: "button",
						className: sidebar_module_css_default.terminalRetry,
						onClick: () => {
		...
		*/
		const SETTINGS_NAV_MARKER = "data-dsh-best-sidebar-settings-nav";
		/**
		...
		*/
		...
		const PLUGIN_TOPIC_URL = "https://github.com/topics/dsh-best-sidebar";
		//#endregion
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:D:\BaiduSyncdisk\obsidian\Github\clyde_dsh\dsh_projects\plugins\dsh-best-sidebar\src\client\SideCardSection.module.css.mjs
		const css$1 = ".BCAj7W_section{flex-direction:column;gap:16px;width:100%;max-width:760px;display:flex}.BCAj7W_intro{color:var(--dsw-alias-label-tertiary);margin:0;padding:0 2px;font-size:13px;line-height:20px}.BCAj7W_group{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:16px;flex-direction:column;flex:none;gap:8px;padding:20px;display:flex}.BCAj7W_groupHeading{color:var(--dsw-alias-label-primary);align-items:baseline;gap:7px;padding:0 2px 6px;font-size:13px;font-weight:600;line-height:20px;display:flex}.BCAj7W_count{background:var(--dsw-alias-accent-soft,var(--dsw-alias-bg-layer-2));color:var(--dsw-alias-label-secondary);font-variant-numeric:tabular-nums;border-radius:999px;padding:1px 8px;font-size:11px;font-weight:500;line-height:16px}.BCAj7W_grid{grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:12px;display:grid}.BCAj7W_card{border:1px solid var(--dsw-alias-border-l2);min-height:106px;font:inherit;color:inherit;cursor:pointer;background:0 0;border-radius:12px;flex-direction:column;transition:background .12s,border-color .12s;display:flex;position:relative;overflow:hidden}.BCAj7W_card:not(.BCAj7W_cardOn):hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-label-dimmed)}.BCAj7W_cardOn{border-color:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 45%, transparent);background:var(--dsw-alias-interactive-bg-active)}.BCAj7W_cardMain{border-radius:inherit;width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;flex-direction:column;flex:1;gap:6px;padding:12px;display:flex}.BCAj7W_cardMain:focus-visible,.BCAj7W_cardSettings:focus-visible,.BCAj7W_rowGear:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:2px}.BCAj7W_cardTop{align-items:center;gap:8px;min-width:0;min-height:28px;display:flex}.BCAj7W_cardIconChip{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);width:28px;height:28px;color:var(--dsw-alias-label-tertiary);border-radius:8px;flex:none;justify-content:center;align-items:center;display:inline-flex}.BCAj7W_cardOn .BCAj7W_cardIconChip{border-color:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 35%, transparent);background:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 12%, transparent);color:var(--dsw-alias-button-primary-fill)}.BCAj7W_cardTitle{min-width:0;color:var(--dsw-alias-label-secondary);white-space:nowrap;text-overflow:ellipsis;flex:1;font-size:13px;font-weight:600;line-height:20px;overflow:hidden}.BCAj7W_cardOn .BCAj7W_cardTitle{color:var(--dsw-alias-label-primary)}.BCAj7W_cardSwitch{flex:none;align-items:center;display:inline-flex}.BCAj7W_cardSwitchTrack{box-sizing:border-box;background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;align-items:center;width:30px;height:16px;padding:2px;transition:background .15s,border-color .15s;display:inline-flex}.BCAj7W_cardSwitchThumb{background:var(--dsw-alias-label-tertiary);border-radius:50%;width:10px;height:10px;transition:transform .15s,background .15s;display:block}.BCAj7W_cardOn .BCAj7W_cardSwitchTrack{border-color:var(--dsw-alias-button-primary-fill);background:var(--dsw-alias-button-primary-fill)}.BCAj7W_cardOn .BCAj7W_cardSwitchThumb{background:var(--dsw-alias-bg-layer-3);transform:translate(14px)}.BCAj7W_cardDesc{color:var(--dsw-alias-label-tertiary);white-space:nowrap;text-overflow:ellipsis;font-size:11px;line-height:16px;overflow:hidden}.BCAj7W_addCard{border-style:dashed;border-color:var(--dsw-alias-border-l2);text-align:left;align-items:flex-start;padding:12px}.BCAj7W_addCard:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}.BCAj7W_addCard:hover .BCAj7W_cardTitle{color:var(--dsw-alias-label-primary)}.BCAj7W_addCard:hover .BCAj7W_cardIconChip{border-color:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 35%, transparent);color:var(--dsw-alias-button-primary-fill)}.BCAj7W_addCard:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:2px}.BCAj7W_cardOn .BCAj7W_cardDesc{color:var(--dsw-alias-label-secondary)}.BCAj7W_cardSettings{border:0;border-top:1px solid var(--dsw-alias-border-l1);width:100%;color:var(--dsw-alias-label-secondary);font:inherit;text-align:left;cursor:pointer;background:0 0;align-items:center;gap:6px;padding:6px 12px;font-size:11px;font-weight:500;line-height:16px;transition:background .12s,color .12s;display:flex}.BCAj7W_cardOn .BCAj7W_cardSettings{border-top-color:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 18%, transparent)}.BCAj7W_cardSettings:hover{background:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-brand-primary)}.BCAj7W_rowGear{border:1px solid var(--dsw-alias-border-l2);width:22px;height:22px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border-radius:6px;flex:none;justify-content:center;align-items:center;padding:0;transition:background .12s,border-color .12s,color .12s;display:inline-flex}.BCAj7W_rowGear:hover{border-color:var(--dsw-alias-interactive-bg-hover-accent);background:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-brand-primary)}.BCAj7W_row{border-bottom:1px solid var(--dsw-alias-border-l2);justify-content:space-between;align-items:center;gap:16px;padding:12px 2px;display:flex}.BCAj7W_row:last-child{border-bottom:none}.BCAj7W_rowText{flex-direction:column;gap:4px;min-width:0;display:flex}.BCAj7W_title{color:var(--dsw-alias-label-primary);font-size:14px;line-height:22px}.BCAj7W_desc{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px}.BCAj7W_switch{cursor:pointer;flex:none;display:inline-flex;position:relative}.BCAj7W_switchInput{opacity:0;width:1px;height:1px;margin:0;position:absolute}.BCAj7W_switchTrack{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);border-radius:10px;align-items:center;width:36px;height:20px;padding:2px;transition:background .15s,border-color .15s;display:inline-flex}.BCAj7W_switchThumb{background:var(--dsw-alias-label-tertiary);border-radius:50%;width:14px;height:14px;transition:transform .15s,background .15s;display:block}.BCAj7W_switch:hover .BCAj7W_switchTrack{border-color:var(--dsw-alias-label-dimmed)}.BCAj7W_switchInput:checked+.BCAj7W_switchTrack{border-color:var(--dsw-alias-button-primary-fill);background:var(--dsw-alias-button-primary-fill)}.BCAj7W_switchInput:checked+.BCAj7W_switchTrack .BCAj7W_switchThumb{background:var(--dsw-alias-bg-layer-3);transform:translate(16px)}.BCAj7W_switchInput:focus-visible+.BCAj7W_switchTrack{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}.BCAj7W_control{flex:none;align-items:center;gap:6px;display:flex}.BCAj7W_percentInput{width:76px}.BCAj7W_typedInput{width:200px}.BCAj7W_typedInputNumber{width:76px}.BCAj7W_selectAnchor{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);max-width:220px;color:var(--dsw-alias-label-primary);cursor:pointer;border-radius:8px;align-items:center;gap:6px;padding:4px 8px;font-size:13px;line-height:20px;display:flex}.BCAj7W_selectAnchor:hover{border-color:var(--dsw-alias-label-dimmed)}.BCAj7W_selectAnchorIcon{flex:none;display:inline-flex}.BCAj7W_selectAnchorText{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.BCAj7W_selectOption{align-items:center;gap:10px;min-width:200px;display:flex}.BCAj7W_selectOptionIcon{color:var(--dsw-alias-label-secondary);flex:none;display:inline-flex}.BCAj7W_selectOptionText{flex-direction:column;min-width:0;display:flex}.BCAj7W_suffix{color:var(--dsw-alias-label-secondary);font-size:14px;line-height:22px}.BCAj7W_cssTextArea{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);width:100%;min-height:120px;color:var(--dsw-alias-label-primary);font-family:var(--ds-font-family-code,monospace);resize:vertical;border-radius:8px;padding:8px 10px;font-size:12px;line-height:1.6}.BCAj7W_cssTextArea:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}.BCAj7W_popupDialog.BCAj7W_popupDialog{width:min(460px,100%)}.BCAj7W_popupRows{box-sizing:border-box;scrollbar-width:thin;scrollbar-color:var(--dsw-alias-scrollbar-bg-l2,transparent) transparent;flex-direction:column;gap:8px;width:100%;max-height:min(52vh,440px);padding-right:4px;display:flex;overflow:hidden auto}.BCAj7W_popupRows::-webkit-scrollbar{width:6px}.BCAj7W_popupRows::-webkit-scrollbar-thumb{background:var(--dsw-alias-scrollbar-bg-l2,var(--dsw-alias-border-l2));border-radius:3px}.BCAj7W_popupRows::-webkit-scrollbar-thumb:hover{background:var(--dsw-alias-scrollbar-hover-l2,var(--dsw-alias-label-dimmed))}.BCAj7W_popupRow{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;flex:none;justify-content:space-between;align-items:center;gap:16px;min-width:0;padding:12px 14px;transition:border-color .16s,background .16s;display:flex}.BCAj7W_popupRow:hover{border-color:var(--dsw-alias-label-dimmed)}.BCAj7W_done{appearance:none;font:inherit;cursor:pointer;background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3);border:1px solid #0000;border-radius:8px;padding:5px 14px;font-size:13px;line-height:1.5}.BCAj7W_done:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}.BCAj7W_error{color:var(--dsw-alias-state-error-primary);padding:10px 0 2px;font-size:12px;line-height:17px}.BCAj7W_pluginModal.BCAj7W_pluginModal{width:min(560px,100%)}.BCAj7W_pluginList{flex-direction:column;gap:12px;width:100%;display:flex}.BCAj7W_pluginTopicBtn{appearance:none;border:1px solid var(--dsw-alias-border-l2);width:100%;font:inherit;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);cursor:pointer;border-radius:8px;padding:6px 12px;font-size:12px;line-height:18px}.BCAj7W_pluginTopicBtn:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}.BCAj7W_pluginTopicBtn:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:1px}.BCAj7W_pluginEmpty{color:var(--dsw-alias-label-tertiary);padding:20px 2px;font-size:12px;line-height:18px}.BCAj7W_pluginEntry{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:12px;flex-direction:column;gap:4px;padding:12px;display:flex}.BCAj7W_pluginEntryHead{justify-content:space-between;align-items:center;gap:12px;display:flex}.BCAj7W_pluginEntryActions{flex:none;align-items:center;gap:6px;display:inline-flex}.BCAj7W_pluginJumpBtn{appearance:none;border:1px solid var(--dsw-alias-border-l2);font:inherit;cursor:pointer;color:var(--dsw-alias-label-secondary);background:0 0;border-radius:8px;flex:none;padding:3px 12px;font-size:12px;line-height:1.5}.BCAj7W_pluginJumpBtn:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}.BCAj7W_pluginJumpBtn:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:1px}.BCAj7W_pluginName{appearance:none;min-width:0;font:inherit;color:var(--dsw-alias-label-primary);text-align:left;text-overflow:ellipsis;white-space:nowrap;cursor:pointer;background:0 0;border:0;padding:0;font-size:13px;font-weight:600;line-height:20px;text-decoration:none;overflow:hidden}.BCAj7W_pluginName:hover{color:var(--dsw-alias-button-primary-fill);text-decoration:underline}.BCAj7W_pluginDesc{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px}.BCAj7W_pluginInstall{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);white-space:nowrap;border-radius:8px;padding:6px 10px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;line-height:16px;display:block;overflow-x:auto}.BCAj7W_pluginCopyBtn{appearance:none;font:inherit;cursor:pointer;background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3);border:1px solid #0000;border-radius:8px;flex:none;padding:3px 12px;font-size:12px;line-height:1.5}.BCAj7W_pluginCopyBtn:hover{background:var(--dsw-alias-button-primary-hover)}.BCAj7W_pluginCopyBtn:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}@media (prefers-reduced-motion:reduce){.BCAj7W_card,.BCAj7W_cardSettings,.BCAj7W_cardSwitchTrack,.BCAj7W_cardSwitchThumb,.BCAj7W_rowGear,.BCAj7W_popupRow,.BCAj7W_switchTrack,.BCAj7W_switchThumb{transition:none}}.BCAj7W_versionBadge{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);border-radius:999px;align-self:flex-start;align-items:center;gap:8px;padding:4px 12px 4px 14px;font-size:12px;line-height:18px;display:inline-flex}.BCAj7W_versionBadgeName{color:var(--dsw-alias-label-primary);font-weight:600}.BCAj7W_versionBadgeTag{background:var(--dsw-alias-accent-soft,var(--dsw-alias-border-l2));color:var(--dsw-alias-label-secondary);font-variant-numeric:tabular-nums;border-radius:999px;padding:1px 8px}.BCAj7W_pluginSearch{box-sizing:border-box;appearance:none;border:1px solid var(--dsw-alias-border-l2);width:100%;font:inherit;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-1);border-radius:8px;padding:6px 10px;font-size:12px;line-height:18px}.BCAj7W_pluginSearch::placeholder{color:var(--dsw-alias-label-tertiary)}.BCAj7W_pluginSearch:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:1px}.BCAj7W_pluginEntries{flex-direction:column;gap:10px;max-height:46vh;padding-right:2px;display:flex;overflow:hidden auto}.BCAj7W_pluginGroup{flex-direction:column;gap:8px;display:flex}.BCAj7W_pluginGroupHeading{color:var(--dsw-alias-label-secondary);padding:2px 2px 0;font-size:12px;font-weight:600;line-height:18px}";
		const tagId$1 = "dsh-external/@aphroditee666/dsh-best-sidebar/SideCardSection.module.css";
		...
		var SideCardSection_module_css_default = {
			"cardTop": "BCAj7W_cardTop",
			"card": "BCAj7W_card",
			"pluginGroup": "BCAj7W_pluginGroup",
			"grid": "BCAj7W_grid",
			"rowText": "BCAj7W_rowText",
			"row": "BCAj7W_row",
			"versionBadgeTag": "BCAj7W_versionBadgeTag",
			"rowGear": "BCAj7W_rowGear",
			"pluginEntryActions": "BCAj7W_pluginEntryActions",
			"section": "BCAj7W_section",
			"done": "BCAj7W_done",
			"switchInput": "BCAj7W_switchInput",
			"pluginInstall": "BCAj7W_pluginInstall",
			"selectAnchorIcon": "BCAj7W_selectAnchorIcon",
			"popupRows": "BCAj7W_popupRows",
			"control": "BCAj7W_control",
			"cardDesc": "BCAj7W_cardDesc",
			"switch": "BCAj7W_switch",
			"cardSwitchThumb": "BCAj7W_cardSwitchThumb",
			"cardSettings": "BCAj7W_cardSettings",
			"typedInputNumber": "BCAj7W_typedInputNumber",
			"selectOptionText": "BCAj7W_selectOptionText",
			"cssTextArea": "BCAj7W_cssTextArea",
			"popupDialog": "BCAj7W_popupDialog",
			"selectAnchor": "BCAj7W_selectAnchor",
			"versionBadgeName": "BCAj7W_versionBadgeName",
			"pluginCopyBtn": "BCAj7W_pluginCopyBtn",
			"selectOptionIcon": "BCAj7W_selectOptionIcon",
			"pluginName": "BCAj7W_pluginName",
			"pluginSearch": "BCAj7W_pluginSearch",
			"title": "BCAj7W_title",
			"addCard": "BCAj7W_addCard",
			"switchTrack": "BCAj7W_switchTrack",
			"groupHeading": "BCAj7W_groupHeading",
			"pluginJumpBtn": "BCAj7W_pluginJumpBtn",
			"cardOn": "BCAj7W_cardOn",
			"group": "BCAj7W_group",
			"count": "BCAj7W_count",
			"selectOption": "BCAj7W_selectOption",
			"cardTitle": "BCAj7W_cardTitle",
			"cardSwitchTrack": "BCAj7W_cardSwitchTrack",
			"cardSwitch": "BCAj7W_cardSwitch",
			"pluginTopicBtn": "BCAj7W_pluginTopicBtn",
			"cardMain": "BCAj7W_cardMain",
			"desc": "BCAj7W_desc",
			"intro": "BCAj7W_intro",
			"popupRow": "BCAj7W_popupRow",
			"pluginList": "BCAj7W_pluginList",
			"error": "BCAj7W_error",
			"pluginEmpty": "BCAj7W_pluginEmpty",
			"pluginDesc": "BCAj7W_pluginDesc",
			"pluginGroupHeading": "BCAj7W_pluginGroupHeading",
			"pluginModal": "BCAj7W_pluginModal",
			"percentInput": "BCAj7W_percentInput",
			"typedInput": "BCAj7W_typedInput",
			"pluginEntryHead": "BCAj7W_pluginEntryHead",
			"versionBadge": "BCAj7W_versionBadge",
			"switchThumb": "BCAj7W_switchThumb",
			"selectAnchorText": "BCAj7W_selectAnchorText",
			"suffix": "BCAj7W_suffix",
			"cardIconChip": "BCAj7W_cardIconChip",
			"pluginEntries": "BCAj7W_pluginEntries",
			"pluginEntry": "BCAj7W_pluginEntry"
		};
		//#endregion
		...
		*/
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				const onDrop = (event) => {
					try {
						...
						event.preventDefault();
						event.stopPropagation();
						const box = target.closest("textarea");
						if (box === null) return;
			...
			ctx.effect(() => registerBuiltins(ctx, service, { terminalTitle: () => terminalTitle }), "dsh-best-sidebar: register built-in tabs and viewers");
			...
			try {
				...
				ctx.effect(() => registerSettingsNavIcon(() => t("settingsNav")), "dsh-best-sidebar: settings navigation icon");
