...
const SIDEBAR_PREFS_NS = "@aphroditee666/dsh-best-sidebar";
//#endregion
...
* @module @aphroditee666/dsh-best-sidebar/config
*/
...
function isPluginRoot(dir) {
	...
	try {
		return JSON.parse(readFileSync(file, "utf8")).name === "@aphroditee666/dsh-best-sidebar";
	} catch {
		return false;
	}
}
...
function createJobOutputMirror(ctx) {
	...
	ctx.effect(() => dispose, "dsh-best-sidebar: job-output event mirror");
...
const name = "@aphroditee666/dsh-best-sidebar";
	...
	return {
		...
		"fs.open": async (payload) => {
			...
			try {
				...
				else if (process.platform === "darwin") spawn("open", [path], {
					detached: true,
					stdio: "ignore"
				}).unref();
		...
		"fs.reveal": async (payload) => {
			...
			const path = requireAbsolute(requireString(payload, "path"));
			try {
				if (process.platform === "win32") spawn("explorer", [`/select,${path}`], {
					detached: true,
					stdio: "ignore"
				}).unref();
				else if (process.platform === "darwin") spawn("open", ["-R", path], {
					detached: true,
					stdio: "ignore"
				}).unref();
				else spawn("xdg-open", [dirname(path)], {
					detached: true,
					stdio: "ignore"
				}).unref();
			} catch (error) {
