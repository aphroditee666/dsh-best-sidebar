...
| **右键「在文件管理器中打开」** | 任意文件/文件夹（含工作区根）右键 → 在系统文件管理器中打开并**选中**它（宿主 `fs.reveal`：Windows `explorer /select`、macOS `open -R`、Linux `xdg-open`） |
| **右键「在应用中打开」** | 右键文件 → 用**系统默认应用**打开（Excel 打开 `.xlsx/.xls`、PowerPoint 打开 `.pptx/.ppt`、PDF/文本等同理；宿主 `fs.open`：Windows `start`、macOS `open`、Linux `xdg-open`） |
